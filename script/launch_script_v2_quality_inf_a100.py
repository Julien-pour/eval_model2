...
python inference_vllm.py --base_path $WORK/eval_model2/ -e {e} -b 2 --arg_gpu "a100" -a 4 --lr={lr} --test_base_model {test_base_model} --arg_bs_test 1024 --arg_model_id {model_id} -k {k} --n_gpu {n_gpu}
"""
...
# list_model_id=["CodeQwen1.5-7B-Chat","CodeQwen1.5-7B-Chat-AWQ","deepseek-coder-6.7B-instruct-AWQ","deepseek-coder-6.7B-instruct-GPTQ","Nous-Hermes-2-Mixtral-8x7B-DPO-AWQ","Nous-Hermes-2-Mixtral-8x7B-DPO-GPTQ"][4:]
# list_model_name=["qwen","qwen_awq","codawq","cod_gptq","mix_awq","mix_gptq"][4:]
...
for id,model_id in enumerate(list_model_id):
    if list_model_name[id] in list_name_slow_model:
        k=10
    elif list_model_name[id] in list_name_slowplus_model:
        k=5
    else:
        k=30
    test_base_model="True"
