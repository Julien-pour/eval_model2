...
for id_model,model_id in enumerate(["deepseek-coder-1.3b-instruct","Meta-Llama-3-8B-Instruct"]):
    for path_test_archive in ["test_archive_diff_50.json","test_archive_diff_75.json","test_archive_diff_85.json","test_archive_diff_80.json"]:
        if id_model==0:
            script_2="""
        python finetune.py --base_path $WORK/eval_model2/ --path_archive "archives/{name_archive}" -e {e} -b 4 --arg_gpu "v100" -a 2 --lr={lr} --test_base_model {test_base_model} --arg_model_id {model_id}
        conda deactivate
        ...
        for name in list_name:
            for seed in list_seed:
                ...
                script_formated = script_1.format(n_gpu=n_gpu,dev_script=dev_script,h="2")+script_2.format(name_archive=archive,e=e,lr=lr,test_base_model=test_base_model,model_id=model_id,model_id_2=model_id,seed=seed,n_gpu_inference=n_gpu,test_archive=path_test_archive)
